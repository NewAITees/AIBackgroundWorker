...
最終更新: 2025-01-27 (実装状況確認・完了タスク更新・次期開発タスク明確化)
...
### CLIビューアーの拡張
- [x] info_collectorデータの表示機能（ニュース、RSS、検索結果、レポート） ✅
  - [x] viewer_service（Web UI）での表示機能は実装済み ✅
  - [x] CLIビューアー（cli_viewer.py）での表示機能は実装済み ✅
    - [x] `news`コマンド（最新ニュース一覧）
    - [x] `reports`コマンド（生成されたレポート一覧）
    - [x] `dashboard`コマンド（統合ダッシュボード）
    - [x] `view`コマンド（今日のサマリー、デフォルト）
    - [x] `recent`コマンド（直近N時間の活動）
- [x] ブラウザ履歴表示機能 ✅
...
- [ ] エラーハンドリングの強化
- [ ] ログローテーション機能
- [x] **イベント情報の収集・格納機能（EVENTVIEW対応）** 🎯 ✅
  - [x] Windowsイベントログやシステムイベント（エラー、警告、重要な操作など）を収集 ✅
  - [x] 現在の`activity_intervals`テーブルに加えて、イベント情報を格納するテーブルを追加 ✅
  - [x] エラー、システムイベント、アプリケーションイベントなど、PCで何が起きているかを把握できる情報を収集 ✅
  - [x] データベーススキーマの拡張（`system_events`テーブルなど） ✅
  - [x] イベント収集コレクターの実装（Windows EventLog API、Linux syslog等） ✅
  - [x] イベント情報の分類・重要度判定機能 ✅
  - [x] 既存の`activity_intervals`と統合した時系列表示（`unified_timeline`ビュー） ✅
  - [ ] イベント収集の定期実行スクリプト・systemd設定（未実装）
...
#### デイリーレポートの包括的統合 🎯
- [x] **包括的なデイリーレポート生成機能の実装** ✅
  - [x] 現在のテーマレポートに加えて、すべての情報を統合したデイリーレポートを生成 ✅
  - [x] 含める情報:
    - [x] ライフログデータ（アクティブウィンドウ、操作状況、時間帯別活動） ✅
    - [x] イベント情報（エラー、システムイベント、重要な操作） ✅
    - [x] ブラウザ履歴（訪問したサイト、時間帯別アクセス） ✅
    - [x] 記事分析結果（重要度・関連度スコア、判断理由） ✅
    - [x] 深掘り調査結果（検索結果、統合分析） ✅
    - [x] テーマレポート（既存のテーマ別レポート） ✅
  - [x] 時間の粒度と意味の明確化:
    - [x] 各情報のタイムスタンプを統一フォーマットで表示 ✅
    - [x] 時間帯別の活動パターンを可視化 ✅
    - [x] イベントと活動の相関関係を分析 ✅
    - [x] 詳細度の調整（サマリー/詳細/完全版の3段階） ✅
  - [x] 人間が把握しやすい形式:
    - [x] 時系列での統合表示 ✅
    - [x] カテゴリ別の整理 ✅
    - [x] 重要度順の並び替え ✅
    - [x] 視覚的な区切りと見出し構造 ✅
  - [x] 既存の`generate_daily_report.py`を拡張または新規実装 ✅
    - [x] `generate_integrated_report.py` を実装 ✅
    - [x] `data_aggregator.py` を実装 ✅
  - [x] プロンプト設計の改善（すべての情報源を考慮した統合プロンプト） ✅
  - [ ] 包括的デイリーレポート生成の定期実行スクリプト・systemd設定（未実装）
...
1. **イベント情報の収集・格納機能（EVENTVIEW対応）** 🎯 ⭐
   ...
   - 実装状況:
     - [x] データベーススキーマの拡張（`system_events`テーブル） ✅
     - [x] イベント収集コレクターの実装 ✅
     - [x] 既存の`activity_intervals`との統合（`unified_timeline`ビュー） ✅
   - **残りのタスク**:
     - [ ] イベント収集の定期実行スクリプト作成
     - [ ] systemdタイマー/サービスの設定
     - [ ] CLIビューアーでのイベント表示機能追加
...
2. **包括的なデイリーレポート生成機能** 🎯 ⭐
   ...
   - 実装状況:
     - [x] 既存の`generate_daily_report.py`を拡張 ✅
     - [x] `generate_integrated_report.py` を実装 ✅
     - [x] `data_aggregator.py` を実装 ✅
     - [x] すべての情報源を統合したプロンプト設計 ✅
     - [x] 時間帯別の活動パターン可視化 ✅
   - **残りのタスク**:
     - [ ] 包括的デイリーレポート生成の定期実行スクリプト作成
     - [ ] systemdタイマー/サービスの設定（日次実行）

3. **ブラウザ履歴機能の完成** ⭐ ✅ 完了
   - [x] `src.browser_history.BraveHistoryImporter` の実装確認 ✅
   - [x] CLIビューアーでの表示機能追加 ✅
...
**全体進捗**: 約 78%（CLIビューアー拡張、イベント収集機能、包括的レポート生成機能の実装により上昇）

---

## 🚀 次の具体的アクション（優先順位順）

### 【最優先】イベント収集の定期実行機能の実装 🎯 ⭐
**目的**: イベント収集コレクターは実装済みだが、定期実行の仕組みが未実装

**実装内容**:
- [ ] イベント収集スクリプトの作成
  - `scripts/lifelog/collect_events.sh` を作成
  - Windows/Linux環境を自動検出して適切なコレクターを使用
  - 設定ファイル（`config/event_collection.yaml`）を読み込み
  - 収集したイベントをDBに保存
- [ ] systemdタイマー/サービスの設定
  - `scripts/systemd/event-collector.service` を作成
  - `scripts/systemd/event-collector.timer` を作成（15分ごと実行推奨）
  - 既存のsystemd設定と同様のパターンで実装
- [ ] CLIビューアーでのイベント表示機能追加
  - `cli_viewer.py` に `events` コマンドを追加
  - 日別・重要度別・タイプ別のフィルタリング機能

**参考実装**:
- 既存: `lifelog-system/src/lifelog/collectors/event_collector.py`（コレクター実装済み）
- 既存: `scripts/systemd/info-collector.service`（systemd設定の参考）
- 既存: `lifelog-system/config/event_collection.yaml`（設定ファイル）

### 【高優先度】包括的デイリーレポート生成の定期実行機能の実装 🎯 ⭐
**目的**: 包括的デイリーレポート生成機能は実装済みだが、定期実行の仕組みが未実装

**実装内容**:
- [ ] 包括的レポート生成スクリプトの作成
  - `scripts/info_collector/generate_integrated_report.sh` を作成
  - `generate_integrated_report.py` を呼び出すラッパースクリプト
  - 日次実行（前日のデータを対象）
- [ ] systemdタイマー/サービスの設定
  - `scripts/systemd/info-integrated-report.service` を作成
  - `scripts/systemd/info-integrated-report.timer` を作成（毎日午前1時実行推奨）
  - 既存のsystemd設定と同様のパターンで実装

**参考実装**:
- 既存: `lifelog-system/src/info_collector/jobs/generate_integrated_report.py`（レポート生成実装済み）
- 既存: `scripts/info_collector/generate_report.sh`（既存レポート生成スクリプトの参考）
- 既存: `scripts/systemd/info-report.timer`（systemd設定の参考）

### 【第一ライン】気軽な履歴確認機能の実装（CLI版）⭐ ✅ 完了（2025-01-27確認）
...
**実装方針**:
- [x] **プロジェクトルートから実行できるラッパースクリプトを作成** ✅
  - [x] `scripts/view.sh` を作成（プロジェクトルートから実行可能） ✅
  - [x] 引数なしで実行したら「今日のサマリー」を表示（デフォルト） ✅
  - [x] サブコマンドで各機能にアクセス（`view`, `recent`, `news`, `reports`, `dashboard`など） ✅
  - [x] 既存の `scripts/lifelog/get_daily_summary.sh` と同じパターン ✅

- [ ] **エイリアス設定の提案（オプション）**
  - `.bashrc` や `.zshrc` に `alias view='cd /home/perso/analysis/AIBackgroundWorker && ./scripts/view.sh'` を追加
  - どこからでも `view` コマンドで実行可能に

### 2. CLIビューアーでのinfo_collector表示機能（1-2日）⭐ ✅ 完了
```bash
# ✅ 実装完了
# - lifelog-system/src/lifelog/cli_viewer.py に追加済み

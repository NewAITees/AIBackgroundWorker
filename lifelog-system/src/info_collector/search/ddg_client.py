...
上記の出力形式に従って、各検索結果の関連性を判定してください。"""
                ...
                try:
                    ...
                    if isinstance(parsed, dict) and "results" in parsed:
                        for item in parsed["results"]:
                            ...
                            if 0 <= idx < len(batch):
                                if item.get("relevant", False) and item.get("relevance_score", 0.0) >= min_relevance_score:
                                    # 関連性スコアを結果に追加
                                    result_with_score = batch[idx].copy()
                                    result_with_score["relevance_score"] = item.get("relevance_score", 0.0)
                                    result_with_score["relevance_reason"] = item.get("reason", "")
                                    relevant_results.append(result_with_score)
                except json.JSONDecodeError:
                    logger.warning("Failed to parse relevance filter response as JSON, keeping all results in batch")
                    # JSONパース失敗時は全件保持（安全策）
    ...
    logger.info("Filtered %d results to %d relevant results (min_score=%.2f)", 
                len(results), len(relevant_results), min_relevance_score)
